...
class RelayActuator(Actuator):
    """Relay digital actuator driver supporting active_high and active_low logic."""
    ...
    async def start(self) -> None:
        try:
            ...
            self._logger.info(
                f"Relay initialized on pin {self.pin} (active_low={self.active_low})"
            )
        except Exception as e:
            self.set_status(DeviceStatus.ERROR, error=f"Failed to initialize Relay: {e}")
    ...
    async def turn_on(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.set_status(DeviceStatus.OK)
            self._logger.info(f"Relay {self.id} -> ON")
        except Exception as e:
            self.set_status(DeviceStatus.ERROR, error=f"Error setting Relay ON: {e}")
    ...
    async def turn_off(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self._last_timestamp = time.time()
            self.set_status(DeviceStatus.OK)
            self._logger.info(f"Relay {self.id} -> OFF")
        except Exception as e:
            self.set_status(DeviceStatus.ERROR, error=f"Error setting Relay OFF: {e}")
